    ...
    public class StoreSettingsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetStoreSettings()
        {
            try
            {
                ...
                {
                    id = config.Id,
                    ten_cua_hang = config.TenCuaHang,
                    logo_url = config.LogoUrl,
                    favicon_url = config.FaviconUrl,
                    so_dien_thoai = config.SoDienThoai,
                    email = config.Email,
                    dia_chi = config.DiaChi,
                    facebook_url = config.FacebookUrl,
                    zalo = config.Zalo,
                    chinh_sach_doi_tra = config.ChinhSachDoiTra,
                    chinh_sach_bao_mat = config.ChinhSachBaoMat,
                    tiktok_url = config.TiktokUrl,
                    instagram_url = config.InstagramUrl,
                    vietqr_bank_bin = config.VietqrBankBin,
                    vietqr_account_no = config.VietqrAccountNo,
                    vietqr_account_name = config.VietqrAccountName,
                    bao_tri_he_thong = config.BaoTriHeThong,
                    tu_dong_duyet_don = config.TuDongDuyetDon,
                    cho_phep_danh_gia = config.ChoPhepDanhGia,
                    gui_email_tu_dong = config.GuiEmailTuDong,
                    lam_tron_tien = config.LamTronTien,
                    smtp_host = config.SmtpHost,
                    smtp_port = config.SmtpPort,
                    smtp_user = config.SmtpUser,
                    smtp_pass = string.IsNullOrEmpty(config.SmtpPass) ? "" : "••••••••",
                    email_nhan_thong_bao = config.EmailNhanThongBao,
                    nguong_bao_het_hang = config.NguongBaoHetHang,
